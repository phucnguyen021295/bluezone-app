/**
 * Copyright 2016-present, Bkav, Cop.
 * All rights reserved.
 *
 * This source code is licensed under the Bkav license found in the
 * LICENSE file in the root directory of this source tree. An additional grant
 * of patent rights can be found in the PATENTS file in the same directory.
 *
 * @author phucnhb@bkav.com on 5/21/20.
 *
 * History:
 * @modifier abc@bkav.com on xx/xx/xxxx đã chỉnh sửa abcxyx (Chỉ các thay đổi quan trọng mới cần ghi lại note này)
 */
'use strict';

export const Images = [
  {
    uri: require('./1.jpg'),
    width: 960,
    height: 720,
    address: 'Biển Hải Dương - Thừa Thiên Huế - Việt Nam',
    addressEn: 'Hai Duong beach - Thua Thien Hue province - Viet Nam',
  },
  {
    uri: require('./2.jpg'),
    width: 720,
    height: 494,
    address: 'Biển Hải Dương - Thừa Thiên Huế - Việt Nam',
    addressEn: 'Hai Duong beach - Thua Thien Hue province - Viet Nam',
  },
  {
    uri: require('./3.jpg'),
    width: 720,
    height: 512,
    address: 'Biển Hải Dương - Thừa Thiên Huế - Việt Nam',
    addressEn: 'Hai Duong beach - Thua Thien Hue province - Viet Nam',
  },
  {
    uri: require('./4.jpg'),
    width: 1600,
    height: 1200,
    address: 'Đồi Móng Ngựa - Mù Cang Chải - Yên Bái - Việt Nam',
    addressEn: 'Mong Ngua hill - Mu Cang Chai - Yen Bai province - Viet Nam',
  },
  {
    uri: require('./5.jpg'),
    width: 960,
    height: 720,
    address:
      'Đường lên Đỉnh Ky Quan San (trước kia gọi là Bạch Mộc Lương Tử) - đỉnh núi cao thứ 4 Việt Nam',
    addressEn:
      'Trekking Ky Quan San mountain (Bach Moc Luong Tu) - The 4th highest mountain peak in Vietnam',
  },
  {
    uri: require('./6.jpg'),
    width: 960,
    height: 720,
    address: 'Cầu tre Cẩm Đồng - Quảng Nam - Việt Nam',
    addressEn: 'Cam Dong bamboo bridge - Quang Nam province - Viet Nam',
  },
  {
    uri: require('./7.jpg'),
    width: 1080,
    height: 808,
    address: 'Đà Lạt - Lâm Đồng - Việt Nam',
    addressEn: 'Da Lat - Lam Dong province - Viet Nam',
  },
  {
    uri: require('./8.jpg'),
    width: 960,
    height: 664,
    address: 'Đà Lạt - Lâm Đồng - Việt Nam',
    addressEn: 'Da Lat - Lam Dong province - Viet Nam',
  },
  {
    uri: require('./9.jpg'),
    width: 961,
    height: 720,
    address: 'Đà Lạt - Lâm Đồng - Việt Nam',
    addressEn: 'Da Lat - Lam Dong province - Viet Nam',
  },
  {
    uri: require('./10.jpg'),
    width: 960,
    height: 540,
    address: 'Đỉnh Mã Pì Lèng - Hà Giang - Việt Nam',
    addressEn: 'The peak of Ma Pi Leng - Ha Giang province - Viet Nam',
  },
  {
    uri: require('./11.jpg'),
    width: 720,
    height: 540,
    address: 'Vịnh Hạ Long - Quảng Ninh - Việt Nam',
    addressEn: 'Ha Long Bay - Quang Ninh province - Viet Nam',
  },
  {
    uri: require('./12.jpg'),
    width: 720,
    height: 540,
    address: 'Vịnh Hạ Long - Quảng Ninh - Việt Nam',
    addressEn: 'Ha Long Bay - Quang Ninh province - Viet Nam',
  },
  {
    uri: require('./13.jpg'),
    width: 960,
    height: 717,
    address: 'Chiều hoàng hôn hồ Đồng Mô - Hà Nội',
    addressEn: 'Sunset on Dong Mo lake - Ha Noi - Viet Nam',
  },
  {
    uri: require('./14.jpg'),
    width: 960,
    height: 720,
    address: 'Lò gạch cũ - huyện Duy Xuyên - Quảng Nam - Việt Nam',
    addressEn:
      'A brickyard - Duy Xuyen district - Quang Nam province - Viet Nam',
  },
  {
    uri: require('./15.jpg'),
    width: 1600,
    height: 1200,
    address: 'Nhà hát Cao Văn Lầu - Bạc Liêu - Việt Nam',
    addressEn: 'Cao Van Lau Theater - Bac Lieu province - Viet Nam',
  },
  {
    uri: require('./16.jpg'),
    width: 720,
    height: 510,
    address: 'Sapa - Lào Cai - Việt Nam',
    addressEn: 'Sapa - Lao Cai province - Viet Nam',
  },
  {
    uri: require('./17.jpg'),
    width: 720,
    height: 496,
    address: 'Sapa - Lào Cai - Việt Nam',
    addressEn: 'Sapa - Lao Cai province - Viet Nam',
  },
  {
    uri: require('./18.jpg'),
    width: 720,
    height: 512,
    address: 'Thung lũng Mường hoa - Sapa - Lao Cai - Việt Nam',
    addressEn: 'Muong Hoa valley - Sapa - Lao Cai province - Viet Nam',
  },
  {
    uri: require('./19.jpg'),
    width: 1600,
    height: 1200,
    address: 'Lăng Cô - Thừa Thiên Huế - Việt Nam',
    addressEn: 'Lang Co - Thua Thien Hue province - Viet Nam',
  },
  {
    uri: require('./20.jpg'),
    width: 961,
    height: 720,
    address: 'Quảng Nam - Việt Nam',
    addressEn: 'Quang Nam - Viet Nam',
  },
];

export const dataVi = [
  {
    content:
      'Không cần phải đốt sách để phá hủy một nền văn hóa. Chỉ cần buộc người ta ngừng đọc mà thôi.',
    user: 'Mahatma Gandhi',
  },
  {
    content:
      'Tôi càng đọc, tôi càng suy ngẫm; và tôi càng được nhiều, tôi càng có thể tin chắc mình không biết điều gì.',
    user: 'Voltaire',
  },
  {
    content:
      'Bầu ơi, thương lấy bí cùng,\n' +
      'Tuy rằng khác giống nhưng chung một giàn',
    user: 'Ca dao',
  },
  {
    content: 'Thua thầy một vạn không bằng kém bạn một ly',
    user: 'Tục ngữ',
  },
  {
    content: 'Lên non đón gió lấy trầm\n' + 'Xui ong lấy mật, giục tầm nhà tơ.',
    user: 'Ca dao',
  },
  {
    content: 'Sách cũng giống như bạn hữu, nên ít và nên được lựa chọn kỹ.',
    user: 'Samuel Johnson',
  },
  {
    content:
      'Hãy có trách nhiệm với cuộc đời mình. Hãy biết rằng chính bạn là người sẽ đưa bạn tới nơi bạn muốn đến chứ không phải ai khác.',
    user: 'Les Brown',
  },
  {
    content: 'Luôn luôn có ánh sáng đằng sau những đám mây.',
    user: 'Louisa May Alcott',
  },
  {
    content: 'Ở đây gần bạn gần thầy\n' + 'Có công mài sắt có ngày nên kim.',
    user: 'Ca dao',
  },
  {
    content: 'Tay làm hàm nhai, tay quai miệng trễ.',
    user: 'Tục ngữ',
  },
  {
    content:
      'Ai mà phụ nghĩa quên công\n' + 'Thì đeo trăm cánh hoa hồng chång thơm',
    user: 'Ca dao',
  },
  {
    content:
      'Chim khôn kêu tiếng rảnh rang\n' +
      'Người khôn nói tiếng dịu dàng dễ nghe,',
    user: 'Ca dao',
  },
  {
    content:
      'Niềm hy vọng là điều rất quan trọng. Nó giúp cho hiện tại bớt khắc nghiệt. Nếu ta hy vọng rằng ngày mai sẽ tốt đẹp hơn, ta sẽ chịu đựng được khó khăn của ngày hôm nay.',
    user: 'Thích Nhất Hạnh',
  },
  {
    content:
      'Lạc quan là niềm tin dẫn tới thành tựu. Bạn chẳng thể làm được điều gì mà thiếu đi hy vọng và sự tự tin.',
    user: 'Helen Keller',
  },
  {
    content:
      'Muốn no thì phải chăm làm\n' + 'Một hạt thóc vàng, chín giọt mô hôi',
    user: 'Ca dao',
  },
  {
    content:
      'Nhiễu điều phủ lấy giá gương\n' +
      'Người trong một nước phải thương nhau cùng',
    user: 'Ca dao',
  },
  {
    content:
      'Con ơi ghi nhớ lời này\n' + 'Công cha, nghĩa mẹ, ơn thầy chở quên.',
    user: 'Ca dao',
  },
  {
    content: 'Một kho vàng không bằng một nang chữ.',
    user: 'Tục ngữ',
  },
  {
    content:
      'Người lạc quan là người nhìn đâu cũng thấy đèn xanh, còn người bi quan thấy khắp nơi chỉ toàn đèn màu đỏ... kẻ thực sự khôn ngoan thì mù màu.',
    user: 'Albert Schweitzer',
  },
  {
    content:
      'Bạn không thể kiểm soát được tình huống xảy ra. Nhưng bạn có thể kiểm soát được suy nghĩ của mình.',
    user: 'Charles Popplestone',
  },
  {
    content:
      'Hãy viết trong trái tim rằng mỗi ngày đều là ngày tuyệt vời nhất trong năm.',
    user: 'Ralph Waldo Emerson',
  },
  {
    content:
      'Khám phá vĩ đại nhất của thời đại chúng ta chính là việc con người có thể thay đổi cuộc sống của mình chính bằng cách thay đổi thái độ sống.',
    user: 'Wiliam James',
  },
  {
    content:
      'Khi một cánh cửa đóng lại, cánh cửa khác sẽ mở ra. Nhưng chúng ta thường nhìn cánh cửa đã đóng đày luyến tiếc đến nỗi chúng ta không nhìn thấy cánh cửa khác đã mở ra cho chúng ta.',
    user: 'Alexander Graham Bell',
  },
  {
    content:
      'Nghèo nhân, nghèo nghĩa thì lo\n' +
      'Nghèo tiền, nghèo bạc chẳng cho là nghèo.',
    user: 'Ca dao',
  },
  {
    content:
      'Thức lâu mới biết đêm dài\n' + 'Ở lâu mới biết lòng người có nhân,',
    user: 'Ca dao',
  },
  {
    content: 'Người không học như ngọc không mài.',
    user: 'Tục ngữ',
  },
  {
    content:
      'Sông sâu sào ngắn khôn dò\n' + 'Người khôn ít nói, khôn đo tấc lòng.',
    user: 'Ca dao',
  },
  {
    content:
      'Cả người lạc quan và người bi quan đều đóng góp cho xã hội. Người lạc quan phát minh ra máy bay. Người bi quan phát minh ra cái dù.',
    user: 'George Bernard Shaw',
  },
  {
    content: 'Sự bi quan dẫn tới yếu đuối, lạc quan dẫn tới sức mạnh.',
    user: 'William James',
  },
  {
    content:
      'Thay vì so sánh mình với những người may mắn hơn mình, ta nên so sánh mình với số đông con người. Và rồi có vẻ như, chúng ta là những người may mắn.',
    user: 'Helen Keller',
  },
  {
    content: 'Hãy quay về hướng mặt trời, và bạn sẽ không còn thấy bóng tối.',
    user: 'Helen Keller',
  },
  {
    content:
      'Không có gì là vĩnh viễn trên thế giới độc ác này - thậm chí cả những rắc rối của chúng ta.',
    user: 'Charlie Chaplin',
  },
  {
    content:
      'Bạn sẽ sống sót. Và điều tốt sẽ lại xảy ra. Và một ngày kia bạn quay đầu lại và thấy những chuyện này thậm chí chẳng phải là điều tồi tệ.',
    user: 'Dale Carnegie',
  },
  {
    content:
      'Người lạc quan vui sướng kinh ngạc con diều của mình bay cao đến thế nào. Người bi quan ủ rũ sầu lo bao giờ con diều của mình sẽ rớt.',
    user: 'William Arthur Ward',
  },
  {
    content:
      'Thương người, người lại thương ta\n' +
      'Ghét người, người lại hóa ra ghét mình.',
    user: 'Ca dao',
  },
  {
    content: 'Một con ngựa đau cả tàu bỏ cỏ.',
    user: 'Tục ngữ',
  },
  {
    content:
      'Nói chín thì nên làm mười\n' + 'Nói mười làm chín kẻ cười, người chê.',
    user: 'Ca dao',
  },
  {
    content: 'Một giọt máu đào hơn ao nước lã.',
    user: 'Tục ngữ',
  },
  {
    content: 'Có tiền thì hậu mới hay\n' + 'Có trồng cây đức mới dày nền nhân.',
    user: 'Ca dao',
  },
  {
    content:
      'Dẫu xây chín bậc phù đồ \n' + 'Không bằng làm phúc cứu cho một người.',
    user: 'Ca dao',
  },
  {
    content:
      'Giáo dục là vũ khí mạnh nhất mà người ta có thể sử dụng để thay đổi cả thế giới.',
    user: 'N. Mandela',
  },
  {
    content:
      'Cần phải lựa chọn một cách nghiêm ngặt xem nên học gì và không nên học gì.',
    user: 'Lev. Tolstoy',
  },
  {
    content:
      'Tài năng là một thứ hiếm hoi. Cần thận trọng nâng đỡ nó một cách hệ thống.',
    user: 'V.I. Lenin',
  },
  {
    content:
      'Điều tôi muốn biết trước tiên không phải là bạn đã thất bại ra sao mà là bạn đã chấp nhận nó như thế nào',
    user: 'Abraham Lincoln',
  },
  {
    content:
      'Nếu cho tôi 6 tiếng để đốn hạ một cái cây, tôi sẽ dành 4 tiếng đầu tiên để mài rìu.',
    user: 'Abraham Lincoln',
  },
  {
    content:
      'Kẻ nào chưa từng mắc phải lỗi lầm cũng là kẻ chưa bao giờ thử làm việc gì cả.',
    user: 'Albert Einstein',
  },
  {
    content:
      'Học từ ngày hôm qua, sống ngày hôm nay, hi vọng cho ngày mai. Điều quan trọng nhất là không ngừng đặt câu hỏi.',
    user: 'Albert Einstein',
  },
  {
    content: 'Đừng phấn đấu để thành công mà hãy phấn đấu để mình có ích.',
    user: 'Albert Einstein',
  },
  {
    content:
      'Sự khác biệt giữa thiên tài và kẻ ngu dốt là ở chỗ thiên tài luôn có giới hạn.',
    user: 'Albert Einstein',
  },
  {
    content:
      'Đoàn kết, đoàn kết, đại đoàn kết. Thành công, thành công, đại thành công',
    user: 'Bác Hồ',
  },
  {
    content:
      'Các vua Hùng đã có công dựng nước, Bác cháu ta phải cùng nhau giữ lấy nước.',
    user: 'Bác Hồ',
  },
  {
    content:
      'Vì lợi ích mười năm thì phải trồng cây, vì lợi ích trăm năm thì phải trồng người.',
    user: 'Bác Hồ',
  },
  {
    content:
      'Điều gì phải, thì cố làm cho kỳ được, dù là việc nhỏ. Điều gì trái, thì hết sức tránh, dù là một điều trái nhỏ.',
    user: 'Bác Hồ',
  },
  {
    content: 'Để thành công, bạn phải sẵn lòng thất bại.',
    user: 'Frank Tyger',
  },
  {
    content:
      'Đôi khi cuộc đời sẽ ném gạch vào đầu bạn. Đừng đánh mất niềm tin.',
    user: 'Steve Jobs',
  },
  {
    content: 'Thước đo của cuộc đời không phải thời gian, mà là cống hiến.',
    user: 'Peter Marshall',
  },
  {
    content:
      'Cuộc sống là một chuỗi bài học mà bạn cần phải sống mới hiểu được.',
    user: 'Helen Keller',
  },
  {
    content:
      'Tất cả cuộc sống đều là thử nghiệm. Bạn càng thử nghiệm nhiều càng tốt.',
    user: 'Ralph Waldo Emerson',
  },
  {
    content: 'Luôn luôn có cách tốt hơn cho mọi thứ. Hãy tìm nó.',
    user: 'Thomas Edison',
  },
  {
    content:
      'Xây dựng thành công từ thất bại. Sự chán nản và thất bại là hai bước đệm chắc chắn nhất dẫn tới thành công.',
    user: 'Dale Carnegie',
  },
  {
    content:
      'Cái ngày đen tối nhất rồi cũng sẽ trôi qua nếu bạn sống tới ngày mai.',
    user: 'William Cowper',
  },
  {
    content: 'Nếu tất cả mọi người cùng nhau tiến lên, thành công sẽ tự đến.',
    user: 'Henry Ford',
  },
  {
    content:
      'Nếu bạn không thể thất bại, bạn sẽ chẳng bao giờ có thể tiến bộ hơn.',
    user: 'Benedict Cumberbatch',
  },
  {
    content: 'Khi bạn tin mình có thể... bạn có thể!',
    user: 'Maxwell Maltz',
  },
  {
    content: 'Có bột mới gột nên hồ\n' + 'Có vôi có gạch mới tô nên nhà.',
    user: 'Tục ngữ',
  },
  {
    content:
      'Vàng thì thử lửa thử than\n' +
      'Chuông kêu thử tiếng, người ngoan thử lời.',
    user: 'Ca dao',
  },
  {
    content: 'Ruộng bề bề không bằng một nghề trong tay.',
    user: 'Tục ngữ',
  },
  {
    content:
      'Dù ai nói ngả nói nghiêng\n' + 'Lòng ta vẫn vững như kiềng ba chân.',
    user: 'Ca dao',
  },
];

export const dataEn = [
  {
    content:
      "You don't have to burn books to destroy a culture. Just get people to stop reading them.",
    user: 'Mahatma Gandhi',
  },
  {
    content:
      'The more I read, the more I meditate; and the more I acquire, the more I am enabled to affirm that I know nothing.',
    user: 'Voltaire',
  },
  {
    content: 'Books like friends, should be few and well-chosen.',
    user: 'Samuel Johnson',
  },
  {
    content:
      'Accept responsibility for your life. Know that it is you who will get you where you want to go, no one else.',
    user: 'Les Brown',
  },
  {
    content: 'There is always light behind the clouds.',
    user: 'Louisa May Alcott',
  },
  {
    content:
      'Optimism is the faith that leads to achievement. Nothing can be done without hope and confidence.',
    user: 'Helen Keller',
  },
  {
    content:
      'Write it on your heart that every day is the best day in the year.',
    user: 'Ralph Waldo Emerson',
  },
  {
    content:
      'Both optimists and pessimists contribute to society. The optimist invents the aeroplane, the pessimist the parachute.',
    user: 'George Bernard Shaw',
  },
  {
    content: 'Pessimism leads to weakness, optimism to power.',
    user: 'William James',
  },
  {
    content:
      'Instead of comparing our lot with that of those who are more fortunate than we are, we should compare it with the lot of the great majority of our fellow men. It then appears that we are among the privileged.',
    user: 'Helen Keller',
  },
  {
    content: 'Keep your face to the sunshine and you cannot see a shadow.',
    user: 'Helen Keller',
  },
  {
    content:
      'Nothing is permanent in this wicked world - not even our troubles.',
    user: 'Charlie Chaplin',
  },
  {
    content:
      'You are going to survive. And good things are going to start to happen again. And one day you are going to look back and this will not even be such a bad thing.',
    user: 'Dale Carnegie',
  },
  {
    content:
      'The optimist pleasantly ponders how high his kite will fly; the pessimist woefully wonders how soon his kite will fall.',
    user: 'William Arthur Ward',
  },
  {
    content:
      'Education is the most powerful weapon which you can use to change the world.',
    user: 'N. Mandela',
  },
  {
    content: 'A person who never made a mistake never tried anything new.',
    user: 'Albert Einstein',
  },
  {
    content:
      'Learn from yesterday, live for today, hope for tomorrow. The important thing is not to stop questioning.',
    user: 'Albert Einstein',
  },
  {
    content: 'Strive not to be a success, but rather to be of value.',
    user: 'Albert Einstein',
  },
  {
    content:
      'The difference between stupidity and genius is that genius has its limits.',
    user: 'Albert Einstein',
  },
  {
    content: "To be successful, you've got to be willing to fail.",
    user: 'Frank Tyger',
  },
  {
    content:
      "Sometimes life is going to hit you in the head with a brick. Don't lose faith.",
    user: 'Steve Jobs',
  },
  {
    content: 'The measure of life is not its duration, but its donation.',
    user: 'Peter Marshall',
  },
  {
    content:
      'Life is a succession of lessons which must be lived to be understood.',
    user: 'Helen Keller',
  },
  {
    content:
      'All life is an experiment. The more experiments you make the better.',
    user: 'Ralph Waldo Emerson',
  },
  {
    content: 'There is a better way for everything. Find it.',
    user: 'Thomas Edison',
  },
  {
    content:
      'Develop success from failures. Discouragement and failure are two of the surest stepping stones to success.',
    user: 'Dale Carnegie',
  },
  {
    content: 'The darkest day, if you live till tomorrow, will have past away.',
    user: 'William Cowper',
  },
  {
    content:
      'If everyone is moving forward together, then success takes care of itself.',
    user: 'Henry Ford',
  },
  {
    content: "If you can't fail, you can never get better.",
    user: 'Benedict Cumberbatch',
  },
  {
    content: 'When you believe you can... you can!',
    user: 'Maxwell Maltz',
  },
];

export const dayVi = {
  '0': 'Thứ Hai',
  '1': 'Thứ Ba',
  '2': 'Thứ Tư',
  '3': 'Thứ Năm',
  '4': 'Thứ Sáu',
  '5': 'Thứ Bảy',
  '6': 'Chủ Nhật',
};

export const dayEn = {
  '0': 'Monday',
  '1': 'Tuesday',
  '2': 'Wednesday',
  '3': 'Thursday',
  '4': 'Friday',
  '5': 'Saturday',
  '6': 'Sunday',
};

export const monthEn = [
  'Jan',
  'Feb',
  'Mar',
  'Apr',
  'May',
  'Jun',
  'Jul',
  'Aug',
  'Sept',
  'Oct',
  'Nov',
  'Dec',
];

export const monthEnFullName = [
  'January',
  'February',
  'March',
  'April',
  'May',
  'June',
  'July',
  'August',
  'September',
  'October',
  'November',
  'December',
];
